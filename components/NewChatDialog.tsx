...
export function NewChatDialog({ children }: { children: React.ReactNode }) {
  ...
  const creatNewChat = useCreateNewChat();
  const { user } = useUser();
  ...
  const handleCreateChat = async () => {
    ...
    const channel = await creatNewChat({
      members: [
        user?.id as string,
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="sm:max-w-[500px] max-h-[80vh] overflow-auto">
        <DialogHeader>
          <DialogTitle className="mb-4">Start a New Chat</DialogTitle>
          <DialogDescription>
            Search for users to start a new chat with them.
          </DialogDescription>
        </DialogHeader>
